...
test('Verify User is able to create HS Assessment', async ({ page }) => {
  ...
  await page.getByRole('menuitem', { name: 'HS' }).click()
  await expect(page.getByRole('button', { name: 'HS Assessment', exact: true })).toBeVisible();
